...
return {
  {
    ...
    config = function()
      ...
      vim.defer_fn(function()
        langs.install_ensure_installed()
      end, 100)
    end,
  },

  {
    ...
    config = function()
      require("nvim-treesitter.config").setup({
        ensure_installed = langs.get_ensure_installed_parsers(),
...

  {
    ...
    config = function()
      ...
      local on_attach = function(client, bufnr)
        ...
        local lang_name, lang = langs.get_language_for_filetype(ft)
        local formatter = langs.apply_tool_defaults(lang and lang.formatter)
      ...
      for lang_name, lsp in pairs(langs.get_all_lsp_configs()) do
      ...
      require("mason-lspconfig").setup({
        ensure_installed = langs.get_ensure_installed_lsp_servers(),
      ...
      vim.api.nvim_create_user_command("LanguageInstall", function(opts)
        ...
        if lang_name == "" then
          local lang_names = vim.tbl_keys(langs.languages)
          vim.ui.select(lang_names, {
            prompt = "Select language to install:",
          }, function(choice)
            if choice then
              langs.install_language(choice)
            end
          end)
        else
          langs.install_language(lang_name)
        end
      end, {
        nargs = "?",
        complete = function()
          return vim.tbl_keys(langs.languages)
        end,
      ...
      vim.api.nvim_create_user_command("LanguageUninstall", function(opts)
        ...
        if lang_name == "" then
          ...
          vim.ui.select(lang_names, {
            ...
            if choice then
              langs.uninstall_language(choice)
            end
          end)
        else
          langs.uninstall_language(lang_name)
        end
      end, {
        nargs = "?",
        complete = function()
      ...
      vim.api.nvim_create_autocmd({ "BufReadPost", "BufNewFile" }, {
        ...
        callback = function()
          ...
          local status = langs.is_installed(lang_name)
          if not status then
            return
          end
...

  {
    ...
    config = function()
      ...
      for lang_name, formatter in pairs(langs.get_all_formatters()) do
          ...
          local opts = {
            condition = function()
              if formatter_data.mason == false then
                ...
                return langs.is_mason_installed(name)
              end
            end,
          }
      ...
      for lang_name, linter in pairs(langs.get_all_linters()) do
          ...
          local opts = {
            condition = function()
              if linter.mason == false then
                ...
                return langs.is_mason_installed(linter.name)
              end
            end,
          }
